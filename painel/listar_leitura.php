<?php  
	...
	if (isset($_GET['excluir'])) {
		mysqli_query(conecta(), "DELETE FROM livros WHERE id = {$_GET['excluir']}");
		echo "<script>alert('Livro exclu\u00eddo com sucesso');location='listar_leitura.php';</script>";
	}
?>
			...
			<div id="conteudo">
				...
				<div id="acoplacampos">
					...
					<table cellpadding="5px" cellspacing="0" ID="alter">
						...
						<?php
						$consulta = mysqli_query(conecta(), "SELECT * FROM livros");
						$i = 0;
						while($row = mysqli_fetch_object($consulta)) {
							if ($i % 2 == 1) {
								echo "
									<tr class='dif'> 
										<td> <a id='link' href='".getLeituraURL($row->id)."'> {$row->livro} </a> </td>
										<td> ". date('d/m/Y', strtotime($row->data_cad)) ." </td>
										<td> <a href='cadastrar_leitura.php?editar={$row->id}'> <img id='imgeditar' src='_img/cadastrar.png'> </td> </a>
